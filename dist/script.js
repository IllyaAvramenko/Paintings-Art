...
var calc = function calc(size, material, options, promocode, result) {
  ...
  Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResourse"])('http://localhost:3000/sizes').then(function (res) {
    return setSizeValues(res);
  }).catch(function (error) {
    return console.log(error);
  });
  Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResourse"])('http://localhost:3000/materials').then(function (res) {
    return setMaterialsValues(res);
  }).catch(function (error) {
    return console.log(error);
  });
  Object(_services_requests__WEBPACK_IMPORTED_MODULE_1__["getResourse"])('http://localhost:3000/options').then(function (res) {
    return setOptionsValues(res);
  }).catch(function (error) {
    return console.log(error);
  });

  var setSizeValues = function setSizeValues(response) {
    sizeBlock.children.forEach(function (option, i) {
      if (i > 0) {
  ...
  var setMaterialsValues = function setMaterialsValues(response) {
    materialBlock.children.forEach(function (option, i) {
      if (i > 0) {
        option.setAttribute('value', response[i - 1].value);
  ...
  var setOptionsValues = function setOptionsValues(response) {
    optionsBlock.children.forEach(function (option, i) {
      if (i > 0) {
        option.setAttribute('value', response[i - 1].value);
...
var changeFormState = function changeFormState(state) {
  var pictureSize = document.querySelectorAll('#size'),
...
var filter = function filter() {
  ...
  var typeFilter = function typeFilter(markTypeSelector) {
    var markType = wrapper.querySelectorAll(markTypeSelector);
  ...
  var setFilter = function setFilter(markSelector) {
    var btn = menu.querySelector(markSelector);
    btn.addEventListener('click', function () {
  ...
  menu.addEventListener('click', function (e) {
    ...
    if (target && target.tagName == "LI") {
      items.forEach(function (btn) {
        return btn.classList.remove('filter__active');
      });
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      if (item.getAttribute('data-calc') === "styles") {
        state.totalPrice = document.querySelector('.calc-price').textContent;
      ...
      if (item.getAttribute('data-calc') === 'calc') {
        ...
        for (var _key in formChild) {
          if (formChild[_key].nodeName === 'SELECT') {
            formData.append(formChild[_key].id, formChild[_key].options[formChild[_key].options.selectedIndex].outerText);
          } else if (formChild[_key].classList === 'INPUT') {
            formData.append(formChild[_key], formChild[_key].value);
          }
        }
      }
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var animationClass = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 'fadeIn';
...
var getResourse = function getResourse(url) {
  var res;
